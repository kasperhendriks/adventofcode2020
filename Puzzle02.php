...
echo sprintf('Puzzle 2 part 1: %d'.PHP_EOL, (new Puzzle02())->part1());
echo sprintf('Puzzle 2 part 2: %d'.PHP_EOL, (new Puzzle02())->part2());

final class Puzzle02
{
    public function part1(): int
    {
        ...
        foreach ($this->getInput() as $line) {
            [$min, $max, $letter, $password] = $this->parseLine($line);
    ...
    public function part2(): int
    {
        ...
        foreach ($this->getInput() as $line) {
            [$first, $second, $letter, $password] = $this->parseLine($line);
    ...
    {
        preg_match('/(?<first>\w+)-(?<second>\w+)\s(?<letter>\w+):\s(?<password>\w+)/', $line, $matches);
